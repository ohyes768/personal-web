...
加了 `dtype` 之后并不是所有 `zfill` 都能删。按数据来源分三类：

| 类别 | 例子 | 是否保留 |
|------|------|---------|
| **外部边界归一** | `[str(c).zfill(6) for c in body.codes]`（API 入参）<br>`df["代码"].astype(str).str.zfill(6)`（akshare 返回）<br>`str(stock_code).zfill(6)`（公开函数入口） | **必须保留**。数据来源不受我们控制，前端可能传 `90`，akshare 可能返回 int 或 `001220.SZ` |
| **内部重复补偿** | `df["股票代码"].astype(str).str.zfill(6)`，df 读自本地 CSV | 保留。加 dtype 后已是 no-op，但幂等无害，作纵深防御。**清理 70+ 处的收益远小于判错来源的风险** |
| **`str(int(x))` 包裹** | `str(int(row["股票代码"])).zfill(6)` | **已清理**。多一层 `int()` 遇非纯数字会 `ValueError` |

> ⚠️ 删 `zfill` 前必须确认数据源：若列因含 NaN 被推成 float64，
> `str(90.0).zfill(6)` 会得到 `"0090.0"` 而非 `"000090"`。
> 只有确认上游有 `dtype` 保证时才可以删。
...
4. **验证必须做反向对照**：一次 `/m120/status` 返回 `missing_count=0` 看似通过，
   反向删掉 `000090` 后**仍是 0** —— 因为探针数据缺 2023/2024 分红列，
   被 `filter_by_3y_dividend` 全滤掉，`all_codes` 是空集，差集恒为 0。
   **"期望值恰好等于退化值"的验证是假阳性。**
   构造探针数据时要先确认业务过滤条件（本例：3 年平均股息率 ≥ 阈值
   **且 2023/2024/2025 每年都有分红**）。
